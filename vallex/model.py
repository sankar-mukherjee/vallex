...
class VALLF(nn.Module):
    ...
    def forward(
        ...
        if train_stage in [0, 1]:
            ...
            ar_logits = self.ar_predict_layer(y_dec).permute(0, 2, 1)
        ...
        if train_stage in [0, 2]:
            ...
            y_len = y_lens.max()
            nar_targets = codes[..., nar_stage] + NUM_AUDIO_TOKENS * y_mask_int
            if self.prefix_mode in [2, 4]:
                nar_targets = nar_targets
                y_mask = F.pad(y_mask, (y_emb.shape[1] - y_len, 0), value=False)
            elif self.prefix_mode == 1:
                nar_targets = nar_targets[:, prefix_len:]
            else:
                assert prefix_len == 0
            ...
            nar_logits = self.nar_predict_layers[nar_stage - 1](y_dec).permute(
                0, 2, 1
            )
...
class VALLE(VALLF):
    ...
    def forward(
        ...
        if train_stage in [0, 1]:
            ...
            ar_logits = self.ar_predict_layer(xy_dec[:, x_len:]).permute(0, 2, 1)
        ...
        if train_stage in [0, 2]:
            ...
            if self.prefix_mode == 4:
                prefix_len = 0  # reset for Top10Accuracy metric
            nar_logits = self.nar_predict_layers[nar_stage - 1](xy_dec).permute(
                0, 2, 1
            )
        # loss norm
        total_length = (y_lens).sum().type(torch.float32)
        nar_loss_norm_factor = total_length / (total_length - prefix_len * x.shape[0])

        return codes, ar_logits, ar_targets, nar_logits, nar_targets, nar_loss_norm_factor, total_length
