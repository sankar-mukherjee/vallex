...
def log(step, tb_total_steps=None, data={}, subset='train', verbosity=Verbosity.VERBOSE):
    if tb_total_steps is not None:
        tb_loggers[subset].log(tb_total_steps, data)

    if subset != '':
        data = {f'{subset}_{key}': v for key, v in data.items()}
    # dllogger.log(step, data=data, verbosity=verbosity)
